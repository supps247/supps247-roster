...
const defaultState = { revision: 0, updatedAt: null, updatedBy: 'system', employees: SUPPS247_EMPLOYEES, locations: SUPPS247_LOCATIONS, shifts: [] };
...
function setConnection(ok, text){
  ...
  if(el){ el.textContent = text || (ok ? 'Cloud connected' : 'Offline'); el.className = `sync-status ${ok ? 'online' : 'offline'}`; }
}
function editorName(){ return currentUser?.email || 'Admin'; }
...
async function save(){
  ...
  try{
    ...
    saving = false;
    if(queuedSave){ queuedSave=false; save(); }
  }
}
...
function shiftsForWeek(start, employee='all'){
  const s=isoDate(start), e=isoDate(weekEnd(start));
  return state.shifts.filter(x=>x.date>=s && x.date<=e && (employee==='all'||x.employee===employee))
...
function renderRosterBuilder(){
  ...
  document.querySelectorAll('.delete-shift').forEach(b=>b.onclick=()=>{
    if(confirm('Delete this shift?')){ state.shifts=state.shifts.filter(s=>s.id!==b.dataset.id); save(); render(); toast('Shift deleted'); }
  });
}
...
function renderTodayTeam(){
  ...
  $('teamList').innerHTML=shifts.length?shifts.map(s=>`<div class="team-row"><div><strong>${s.employee}</strong><p>${s.location} · ${fmtTime(s.start)}–${fmtTime(s.end)}</p></div><span class="status on">Scheduled</span></div>`).join(''):'<p class="muted">No staff rostered today.</p>';
}
...
function render(){ renderRosterBuilder(); renderTodayTeam(); renderStats(); }

function openShiftDialog(shift=null){
  $('dialogTitle').textContent=shift?'Edit shift':'Add shift'; $('shiftId').value=shift?.id||'';
  $('shiftEmployee').value=shift?.employee||state.employees[0]; $('shiftDate').value=shift?.date||isoDate(selectedWeek);
  $('shiftLocation').value=shift?.location||state.locations[0]; $('shiftStart').value=shift?.start||'09:00'; $('shiftEnd').value=shift?.end||'17:00';
  $('shiftBreak').value=shift?.breakMin??30; $('shiftNotes').value=shift?.notes||''; $('shiftDialog').showModal();
}

function renderBulkRosterRows(){
  const days=['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
  $('bulkRosterBody').innerHTML=state.employees.map(employee=>`
    <tr data-employee="${employee}"><td><strong>${employee}</strong></td>
...
function openBulkRosterDialog(){ $('bulkWeek').value=isoDate(selectedWeek); $('bulkLocation').value=state.locations[0]; renderBulkRosterRows(); $('bulkRosterDialog').showModal(); }
...
$('bulkRosterForm').addEventListener('submit',e=>{
  ...
  $('bulkRosterBody').querySelectorAll('tr').forEach(row=>{
    ...
    row.querySelectorAll('.day-shift').forEach(cell=>{
      ...
      const d=new Date(week); d.setDate(d.getDate()+Number(cell.dataset.day)); const date=isoDate(d);
      ...
      state.shifts.push({id:crypto.randomUUID(),date,employee,location,start,end,breakMin,notes}); added++;
    });
  });
  if(!added && !skipped){ toast('Select at least one employee day'); return; }
  save(); $('bulkRosterDialog').close(); selectedWeek=week; render(); toast(`${added} shifts added${skipped?`, ${skipped} skipped`:''}`);
});

$('shiftForm').addEventListener('submit',e=>{
  e.preventDefault();
  const id=$('shiftId').value;
  const item={id:id||crypto.randomUUID(),employee:$('shiftEmployee').value,date:$('shiftDate').value,location:$('shiftLocation').value,start:$('shiftStart').value,end:$('shiftEnd').value,breakMin:Number($('shiftBreak').value)||0,notes:$('shiftNotes').value.trim()};
  if(item.end<=item.start){ toast('Finish time must be after start time'); return; }
  if(id){ const i=state.shifts.findIndex(s=>s.id===id); state.shifts[i]=item; } else state.shifts.push(item);
  save(); $('shiftDialog').close(); render(); toast(id?'Shift updated':'Shift added');
});
$('closeDialog').onclick=()=> $('shiftDialog').close();
$('cancelDialog').onclick=()=> $('shiftDialog').close();
$('addShift').onclick=()=>openShiftDialog();
$('prevWeek').onclick=()=>{ selectedWeek.setDate(selectedWeek.getDate()-7); render(); };
$('nextWeek').onclick=()=>{ selectedWeek.setDate(selectedWeek.getDate()+7); render(); };
$('employeeFilter').onchange=render;
$('copyWeek').onclick=()=>{
  const previous=new Date(selectedWeek); previous.setDate(previous.getDate()-7);
  const source=shiftsForWeek(previous);
  if(!source.length){ toast('Previous week has no shifts'); return; }
  const existingIds=new Set(shiftsForWeek(selectedWeek).map(s=>`${s.date}|${s.employee}|${s.location}|${s.start}`));
  let added=0;
  source.forEach(s=>{ const d=parseDate(s.date); d.setDate(d.getDate()+7); const copy={...s,id:crypto.randomUUID(),date:isoDate(d)}; const key=`${copy.date}|${copy.employee}|${copy.location}|${copy.start}`; if(!existingIds.has(key)){state.shifts.push(copy);added++;} });
  save(); render(); toast(`${added} shifts copied`);
};
$('clearWeek').onclick=()=>{
  if(!confirm(`Clear all shifts for ${weekLabel(selectedWeek)}?`)) return;
  const s=isoDate(selectedWeek), e=isoDate(weekEnd(selectedWeek)); state.shifts=state.shifts.filter(x=>x.date<s||x.date>e); save(); render(); toast('Week cleared');
};
$('exportRoster').onclick=()=>{
  const shifts=shiftsForWeek(selectedWeek);
  if(!shifts.length){ toast('No shifts to export'); return; }
  ...
  const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download=`supps247-roster-${isoDate(selectedWeek)}.csv`; a.click(); URL.revokeObjectURL(a.href);
};
